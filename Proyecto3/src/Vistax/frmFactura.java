package Vistax;

import Controller.ControlUniversal;
import Controller.VerificacionDeCredito;
import Modelos.Articulo;
import Modelos.Detalle;
import Modelos.Existencia;
import Modelos.Factura;
import Modelos.FormaPago;
import Modelos.Socio;
import Modelos.Tienda;
import Utilerias.interfacex;
import java.util.Calendar;
import java.util.Date;
import java.util.GregorianCalendar;
import java.util.Vector;
import javax.swing.JLabel;
import javax.swing.JOptionPane;
import javax.swing.JTextField;
import javax.swing.table.DefaultTableModel;

/**
 *
 * @author root4142
 */
public class frmFactura extends javax.swing.JInternalFrame {

    private interfacex inter = new interfacex();
    private Vector<JTextField> txtCampos = new Vector<>();
    private Vector<JLabel> lblLabels = new Vector<>();
    private Vector<Detalle> Detalle = new Vector<>();
    private Vector titulos, data = new Vector();
    DefaultTableModel modelo;
    private boolean ins = false, b = false, c = false;
    private float totalBruto = 0, total = 0, limite = 0, lim = 0;
    private ControlUniversal control;
    private Factura factura;
    private Detalle detalle;
    private Existencia exis;
    private Socio socio;

    /**
     * Creates new form frmFactura
     */
    public frmFactura(ControlUniversal cont) {
        control = cont;
        titulos = new Vector();
        titulos.addElement("ID Articulo");
        titulos.addElement("Nombre del Articulo");
        titulos.addElement("Cantidad");
        titulos.addElement("Precio unitario");
        titulos.addElement("ISV");
        titulos.addElement("Total");
        initComponents();
        txtCampos.add(txtFacturaID);
        txtCampos.add(txtFecha);
        txtCampos.add(txtTiendaID);
        txtCampos.add(txtSocioID);
        txtCampos.add(txtObservacion);
        lblLabels.add(lblNombreTienda);
        lblLabels.add(lblNombreSocio);
        lblLabels.add(lblDireccionTienda);
        lblLabels.add(lblEstado);
        modelo = (DefaultTableModel) Tabla.getModel();
        Tabla.setModel(modelo);
        factura = new Factura();
        txtFecha.setText(control.getFechaSistema());
        lblEstado.setText("GRABADA");
        int auto = control.autoUniversal2("Factura", "FacturaID");
        txtFacturaID.setText(String.valueOf(auto));
        txtTiendaID.grabFocus();
    }

    private void Limpiar_Campos() {
        for (JTextField o : txtCampos) {
            o.setText("");
        }
        for (JLabel o : lblLabels) {
            o.setText("");
        }
        txtFecha.setText(control.getFechaSistema());
        lblEstado.setText("GRABADA");
        cboTipo.setEnabled(true);
        cboTipo.setSelectedIndex(0);
        ins = false;
        b = false;
        c = false;
        total = 0;
        limite = 0;
        totalBruto = 0;
        lim = 0;
        int auto = control.autoUniversal2("Factura", "FacturaID");
        txtFacturaID.setText(String.valueOf(auto));
        txtTiendaID.grabFocus();
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jPanel1 = new javax.swing.JPanel();
        txtSocioID = new javax.swing.JTextField();
        jLabel5 = new javax.swing.JLabel();
        txtFacturaID = new javax.swing.JTextField();
        jLabel6 = new javax.swing.JLabel();
        txtTiendaID = new javax.swing.JTextField();
        jLabel3 = new javax.swing.JLabel();
        jLabel11 = new javax.swing.JLabel();
        txtObservacion = new javax.swing.JTextField();
        jLabel7 = new javax.swing.JLabel();
        jLabel4 = new javax.swing.JLabel();
        jLabel8 = new javax.swing.JLabel();
        jLabel1 = new javax.swing.JLabel();
        txtFecha = new javax.swing.JTextField();
        jLabel9 = new javax.swing.JLabel();
        jLabel10 = new javax.swing.JLabel();
        cboTipo = new javax.swing.JComboBox();
        lblDireccionTienda = new javax.swing.JLabel();
        lblEstado = new javax.swing.JLabel();
        lblNombreSocio = new javax.swing.JLabel();
        lblNombreTienda = new javax.swing.JLabel();
        valid = new javax.swing.JButton();
        jPanel2 = new javax.swing.JPanel();
        txtArticuloID = new javax.swing.JTextField();
        jLabel15 = new javax.swing.JLabel();
        jLabel22 = new javax.swing.JLabel();
        txtCantidad = new javax.swing.JTextField();
        jLabel19 = new javax.swing.JLabel();
        jLabel23 = new javax.swing.JLabel();
        jLabel24 = new javax.swing.JLabel();
        jLabel16 = new javax.swing.JLabel();
        chkISV = new javax.swing.JCheckBox();
        lblPrecio = new javax.swing.JLabel();
        lblNombreArticulo = new javax.swing.JLabel();
        lblISV = new javax.swing.JLabel();
        lblTotal = new javax.swing.JLabel();
        jScrollPane1 = new javax.swing.JScrollPane();
        Tabla = new javax.swing.JTable();
        btnQuitarLinea = new javax.swing.JButton();
        btnAgregarLinea = new javax.swing.JButton();
        jButton1 = new javax.swing.JButton();
        jButton2 = new javax.swing.JButton();
        jButton3 = new javax.swing.JButton();

        setTitle("Formulario de Factura.");
        setFrameIcon(new javax.swing.ImageIcon(getClass().getResource("/Vistax/Iconos/icon.jpeg"))); // NOI18N
        setName("frmFactura"); // NOI18N
        try {
            setSelected(true);
        } catch (java.beans.PropertyVetoException e1) {
            e1.printStackTrace();
        }
        setVisible(true);
        getContentPane().setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Encabezado de la Factura."));
        jPanel1.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        txtSocioID.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        txtSocioID.setName("BodegaID"); // NOI18N
        txtSocioID.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txtSocioIDActionPerformed(evt);
            }
        });
        jPanel1.add(txtSocioID, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 130, 90, -1));

        jLabel5.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel5.setText("No. de Factura:");
        jPanel1.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 20, -1, -1));

        txtFacturaID.setBackground(java.awt.Color.white);
        txtFacturaID.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        txtFacturaID.setEnabled(false);
        txtFacturaID.setName("CompraID"); // NOI18N
        jPanel1.add(txtFacturaID, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 20, 90, -1));

        jLabel6.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel6.setText("Codigo de Tienda");
        jPanel1.add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 60, -1, -1));

        txtTiendaID.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        txtTiendaID.setName("ProveedorID"); // NOI18N
        txtTiendaID.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txtTiendaIDActionPerformed(evt);
            }
        });
        jPanel1.add(txtTiendaID, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 80, 90, -1));

        jLabel3.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel3.setText("Codigo de Socio");
        jPanel1.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 110, -1, -1));

        jLabel11.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel11.setText("Observacion :");
        jPanel1.add(jLabel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 170, -1, -1));

        txtObservacion.setName("FacturaID"); // NOI18N
        txtObservacion.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                txtObservacionKeyTyped(evt);
            }
        });
        jPanel1.add(txtObservacion, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 190, 200, -1));

        jLabel7.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel7.setText("Nombre de la Tienda");
        jPanel1.add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(220, 60, -1, -1));

        jLabel4.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel4.setText("Nombre de Socio");
        jPanel1.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(220, 110, -1, -1));

        jLabel8.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel8.setText("Dirección de la Tienda");
        jPanel1.add(jLabel8, new org.netbeans.lib.awtextra.AbsoluteConstraints(490, 70, -1, -1));

        jLabel1.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel1.setText("Año-Mes-Día");
        jPanel1.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 10, -1, -1));

        txtFecha.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txtFecha.setEnabled(false);
        txtFecha.setName("Fecha"); // NOI18N
        jPanel1.add(txtFecha, new org.netbeans.lib.awtextra.AbsoluteConstraints(500, 30, 150, -1));

        jLabel9.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel9.setText("Tipo de Pago");
        jPanel1.add(jLabel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(440, 170, -1, -1));

        jLabel10.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel10.setText("Estado de la Factura");
        jPanel1.add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 170, -1, -1));

        cboTipo.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Contado", "Credito" }));
        jPanel1.add(cboTipo, new org.netbeans.lib.awtextra.AbsoluteConstraints(420, 190, 160, -1));

        lblDireccionTienda.setBackground(java.awt.Color.white);
        lblDireccionTienda.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        lblDireccionTienda.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblDireccionTienda.setName("Estado"); // NOI18N
        lblDireccionTienda.setOpaque(true);
        jPanel1.add(lblDireccionTienda, new org.netbeans.lib.awtextra.AbsoluteConstraints(440, 90, 270, 30));

        lblEstado.setBackground(java.awt.Color.white);
        lblEstado.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        lblEstado.setText("GRABADA");
        lblEstado.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblEstado.setName("Estado"); // NOI18N
        lblEstado.setOpaque(true);
        jPanel1.add(lblEstado, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 190, 130, 30));

        lblNombreSocio.setBackground(java.awt.Color.white);
        lblNombreSocio.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        lblNombreSocio.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblNombreSocio.setName("Estado"); // NOI18N
        lblNombreSocio.setOpaque(true);
        jPanel1.add(lblNombreSocio, new org.netbeans.lib.awtextra.AbsoluteConstraints(160, 130, 270, 30));

        lblNombreTienda.setBackground(java.awt.Color.white);
        lblNombreTienda.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        lblNombreTienda.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblNombreTienda.setName("Estado"); // NOI18N
        lblNombreTienda.setOpaque(true);
        jPanel1.add(lblNombreTienda, new org.netbeans.lib.awtextra.AbsoluteConstraints(160, 80, 270, 30));

        valid.setText("Validadar");
        valid.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                validActionPerformed(evt);
            }
        });
        jPanel1.add(valid, new org.netbeans.lib.awtextra.AbsoluteConstraints(620, 180, 130, -1));

        getContentPane().add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, 760, 240));

        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder("Detalle de la Factura."));
        jPanel2.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        txtArticuloID.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        txtArticuloID.setName("ArticuloID"); // NOI18N
        txtArticuloID.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txtArticuloIDActionPerformed(evt);
            }
        });
        jPanel2.add(txtArticuloID, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 50, 90, -1));

        jLabel15.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel15.setText("Codigo de Articulo");
        jPanel2.add(jLabel15, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 30, -1, -1));

        jLabel22.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel22.setText("Nombre del Articulo");
        jPanel2.add(jLabel22, new org.netbeans.lib.awtextra.AbsoluteConstraints(220, 20, -1, -1));

        txtCantidad.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        txtCantidad.setName("Cantidad"); // NOI18N
        txtCantidad.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txtCantidadActionPerformed(evt);
            }
        });
        jPanel2.add(txtCantidad, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 100, 100, -1));

        jLabel19.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel19.setText("Cantidad");
        jPanel2.add(jLabel19, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 80, -1, -1));

        jLabel23.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel23.setText("Precio");
        jPanel2.add(jLabel23, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 80, -1, -1));

        jLabel24.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel24.setText("Total a Pagar :");
        jPanel2.add(jLabel24, new org.netbeans.lib.awtextra.AbsoluteConstraints(570, 50, -1, -1));

        jLabel16.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jLabel16.setText("ISV");
        jPanel2.add(jLabel16, new org.netbeans.lib.awtextra.AbsoluteConstraints(360, 80, -1, -1));

        chkISV.setText("ISV");
        chkISV.setEnabled(false);
        jPanel2.add(chkISV, new org.netbeans.lib.awtextra.AbsoluteConstraints(280, 100, -1, -1));

        lblPrecio.setBackground(java.awt.Color.white);
        lblPrecio.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        lblPrecio.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblPrecio.setOpaque(true);
        jPanel2.add(lblPrecio, new org.netbeans.lib.awtextra.AbsoluteConstraints(140, 100, 110, 30));

        lblNombreArticulo.setBackground(java.awt.Color.white);
        lblNombreArticulo.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblNombreArticulo.setOpaque(true);
        jPanel2.add(lblNombreArticulo, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 40, 240, 30));

        lblISV.setBackground(java.awt.Color.white);
        lblISV.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        lblISV.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblISV.setOpaque(true);
        jPanel2.add(lblISV, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 100, 110, 30));

        lblTotal.setBackground(java.awt.Color.white);
        lblTotal.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        lblTotal.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        lblTotal.setOpaque(true);
        jPanel2.add(lblTotal, new org.netbeans.lib.awtextra.AbsoluteConstraints(690, 40, 110, 30));

        Tabla.setModel(new javax.swing.table.DefaultTableModel(data,titulos)
            {
                boolean[] canEdit = new boolean [] {
                    false, false, false, false, false, false
                };

                public boolean isCellEditable(int rowIndex, int columnIndex) {
                    return canEdit [columnIndex];
                }
            }
        );
        jScrollPane1.setViewportView(Tabla);

        jPanel2.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 160, 860, 160));

        btnQuitarLinea.setText("Quitar Linea");
        btnQuitarLinea.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnQuitarLineaActionPerformed(evt);
            }
        });
        jPanel2.add(btnQuitarLinea, new org.netbeans.lib.awtextra.AbsoluteConstraints(700, 110, 150, -1));

        btnAgregarLinea.setText("Agregar Linea");
        btnAgregarLinea.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnAgregarLineaActionPerformed(evt);
            }
        });
        jPanel2.add(btnAgregarLinea, new org.netbeans.lib.awtextra.AbsoluteConstraints(500, 110, 150, -1));

        getContentPane().add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 260, 890, 340));

        jButton1.setText("Salir");
        jButton1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton1ActionPerformed(evt);
            }
        });
        getContentPane().add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(790, 190, 100, 50));

        jButton2.setText("Cancelar");
        jButton2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton2ActionPerformed(evt);
            }
        });
        getContentPane().add(jButton2, new org.netbeans.lib.awtextra.AbsoluteConstraints(790, 110, 100, 50));

        jButton3.setText("Grabar");
        jButton3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton3ActionPerformed(evt);
            }
        });
        getContentPane().add(jButton3, new org.netbeans.lib.awtextra.AbsoluteConstraints(790, 30, 100, 50));

        java.awt.Dimension screenSize = java.awt.Toolkit.getDefaultToolkit().getScreenSize();
        setBounds((screenSize.width-925)/2, (screenSize.height-636)/2, 925, 636);
    }// </editor-fold>//GEN-END:initComponents

    private void txtSocioIDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtSocioIDActionPerformed
        socio = new Socio();
        Object o[][] = control.seekUniversal(socio.getVectorData(), "Socio", txtSocioID.getText());
        if (o != null) {
            socio = new Socio(o);

            boolean x = control.seekUniversal("SELECT * FROM TiendaEmpresa where TiendaID=" + txtTiendaID.getText() + " and EmpresaID=" + socio.getEmpresaID() + ";");
            if (x) {
                lblNombreSocio.setText(socio.getNombre());
                c = true;
                limite = (float) (socio.getSueldo() * 0.25);
                txtObservacion.grabFocus();
            } else {
                JOptionPane.showMessageDialog(rootPane, "Esta Tienda no esta asignada a esta Empresa", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }//GEN-LAST:event_txtSocioIDActionPerformed

    private void txtTiendaIDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtTiendaIDActionPerformed
        Tienda tienda = new Tienda();
        Object o[][] = control.seekUniversal(tienda.getVectorData(), "Tienda", txtTiendaID.getText());
        if (o != null) {
            tienda = new Tienda(o);
            lblNombreTienda.setText(tienda.getNombre());
            lblDireccionTienda.setText(tienda.getDireccion());
            b = true;
            txtSocioID.grabFocus();
        }
    }//GEN-LAST:event_txtTiendaIDActionPerformed

    private void validActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_validActionPerformed
        if (!b) {
            JOptionPane.showMessageDialog(rootPane, "Campo Codigo de la Tienda no esta validado", "Error", JOptionPane.ERROR_MESSAGE);
            txtTiendaID.grabFocus();
        } else if (!c) {
            JOptionPane.showMessageDialog(rootPane, "Campo Codigo del Socio no esta validado", "Error", JOptionPane.ERROR_MESSAGE);
            txtSocioID.grabFocus();
        } else if (txtCampos.get(4).getText().isEmpty()) {
            JOptionPane.showMessageDialog(rootPane, "Campo Observacion se encuentra Vacio", "Error", JOptionPane.ERROR_MESSAGE);
            txtObservacion.grabFocus();
        } else {
            String d = "";
            if (cboTipo.getSelectedIndex() == 0) {
                d = "D";
                lim = -1;
            } else {
                d = "G";
            }
            cboTipo.setEnabled(false);
            VerificacionDeCredito verifica = new VerificacionDeCredito(inter.String_Int(txtSocioID.getText()), control);
            if (d.equals("G")) {
                Date date2 = inter.String_Date(socio.getFechaIngreso());
                Calendar c1 = new GregorianCalendar();
                c1.setTime(date2);
                c1.add(Calendar.DAY_OF_MONTH, 30);
                Date da = control.getFechaSistemaSql();
                Calendar c2 = new GregorianCalendar();
                c2.setTime(da);
                if (!c1.before(c2)) {
                    JOptionPane.showMessageDialog(rootPane, "Credito Solo se otorga con una antiguedad mayor a los 30 dias", "Error", JOptionPane.ERROR_MESSAGE);
                    txtSocioID.grabFocus();
                    c = false;
                } else {
                    if (verifica.getTipoCredito() == 3) {
                        JOptionPane.showMessageDialog(rootPane, "Ya hay una Factura Vigente no Cancelado por parte de este Socio", "Error", JOptionPane.ERROR_MESSAGE);
                        txtSocioID.grabFocus();
                    } else if (verifica.getTipoCredito() == 2) {
                        JOptionPane.showMessageDialog(rootPane, "Ya hay un Prestamo Vigente no Cancelado por parte de este Socio", "Error", JOptionPane.ERROR_MESSAGE);
                        txtSocioID.grabFocus();
                    } else if (verifica.getTipoCredito() == 1) {
                        JOptionPane.showMessageDialog(rootPane, "Ya hay un Anticipo Vigente no Cancelado por parte de este Socio", "Error", JOptionPane.ERROR_MESSAGE);
                        txtSocioID.grabFocus();
                    } else {
                        factura = new Factura(inter.String_Int(txtTiendaID.getText()), inter.String_Int(txtFacturaID.getText()), inter.String_Int(txtSocioID.getText()), txtFecha.getText(), d, txtObservacion.getText());
                        ins = true;
                        txtArticuloID.grabFocus();
                    }
                }
            } else {
                ins = true;
                txtArticuloID.grabFocus();
            }
        }
    }//GEN-LAST:event_validActionPerformed

    private void txtObservacionKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtObservacionKeyTyped
        if (txtObservacion.getText().length() >= 100) {
            evt.consume();
        }
    }//GEN-LAST:event_txtObservacionKeyTyped

    private void txtArticuloIDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtArticuloIDActionPerformed
        if (ins) {
            if (!txtArticuloID.getText().isEmpty()) {
                Articulo articulo = new Articulo();
                Object o[][] = control.seekUniversal(articulo.getVectorData(), "Articulo", txtArticuloID.getText());
                if (o != null) {
                    exis = new Existencia();
                    Object oo[][] = control.seekUniversal(articulo.getVectorData(), "Existencia", txtArticuloID.getText() + " AND TiendaID=" + txtTiendaID.getText());
                    if (oo != null) {
                        exis = new Existencia(oo);
                        if (exis.getCantidad() == 0) {
                            JOptionPane.showMessageDialog(rootPane, "No hay existencias de este Articulo en esta Tienda", "Error", JOptionPane.ERROR_MESSAGE);
                            txtArticuloID.grabFocus();
                        } else {
                            articulo = new Articulo(o);
                            lblNombreArticulo.setText(articulo.getNombre());
                            float prec = articulo.getPrecio() + (articulo.getPrecio() * articulo.getUtilidad());
                            lblPrecio.setText(prec + "");
                            if (articulo.getISV() == 1) {
                                chkISV.setSelected(true);
                            } else {
                                chkISV.setSelected(false);
                            }
                            txtCantidad.grabFocus();
                        }
                    } else {
                        JOptionPane.showMessageDialog(rootPane, "El Articulo No existe en esta Tienda", "Error", JOptionPane.ERROR_MESSAGE);
                        txtArticuloID.grabFocus();
                    }
                }
            } else {
                JOptionPane.showMessageDialog(rootPane, "El Campo Articulo ID esta Vacio", "Error", JOptionPane.ERROR_MESSAGE);
                txtArticuloID.grabFocus();
            }
        } else {
            JOptionPane.showMessageDialog(rootPane, "El encabezado de la Factura no esta validado aun!!!", "Error", JOptionPane.ERROR_MESSAGE);
            valid.grabFocus();
        }
    }//GEN-LAST:event_txtArticuloIDActionPerformed

    private void txtCantidadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtCantidadActionPerformed
        int cc = inter.String_Int(txtCantidad.getText());
        if (!txtArticuloID.getText().isEmpty()) {
            if (inter.String_Int(txtCantidad.getText()) > exis.getCantidad()) {
                JOptionPane.showMessageDialog(rootPane, "Solo se pueden Facturar " + exis.getCantidad() + " de este tipo de Articulo", "Error", JOptionPane.ERROR_MESSAGE);
            } else {
                float aa = inter.String_Float(lblPrecio.getText());
                if (chkISV.isSelected()) {
                    float z = (float) (cc * aa * 0.12);
                    lblISV.setText(z + "");
                    totalBruto = (float) ((cc * aa) + z);
                } else {
                    lblISV.setText("0");
                    totalBruto = (float) ((cc * aa) + 0);
                }
            }
        } else {
            JOptionPane.showMessageDialog(rootPane, "Campo Codigo de Articulo se encuentra vacio", "Error", JOptionPane.ERROR_MESSAGE);
            txtArticuloID.grabFocus();
        }
    }//GEN-LAST:event_txtCantidadActionPerformed

    private void btnQuitarLineaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnQuitarLineaActionPerformed
        int x = -1;
        x = Tabla.getSelectedRow();
        if (x < 0) {
            JOptionPane.showMessageDialog(rootPane, "No se puede debes de seleccionar una linea de la tabla");
        } else {
            total = total - Float.valueOf(modelo.getValueAt(x, 5).toString());
            lblTotal.setText(total + "");
            Detalle.remove(x);
            modelo.removeRow(x);
            txtArticuloID.setText("");
            txtCantidad.setText("");
            lblNombreArticulo.setText("");
            lblPrecio.setText("");
            lblISV.setText("");
            chkISV.setSelected(false);
            txtArticuloID.grabFocus();
        }
    }//GEN-LAST:event_btnQuitarLineaActionPerformed

    private void btnAgregarLineaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAgregarLineaActionPerformed
        if ((txtArticuloID.getText().isEmpty()) || (txtCantidad.getText().isEmpty())) {
            JOptionPane.showMessageDialog(rootPane, "No se puede ingresar linea con campos vacios");
        } else if (lblISV.getText().isEmpty()) {
            JOptionPane.showMessageDialog(rootPane, "No se puede ingresar linea tiene que validar el campo cantidad");
            txtCantidad.grabFocus();
        } else {
            float temp = total + totalBruto;
            if (lim == -1) {
                limite += temp;
            }
            if (temp <= limite) {
                Vector rou = new Vector();
                rou.addElement(txtArticuloID.getText());
                rou.addElement(lblNombreArticulo.getText().toUpperCase());
                rou.addElement(txtCantidad.getText());
                rou.addElement(lblPrecio.getText());
                rou.addElement(lblISV.getText());
                rou.addElement(totalBruto);
                total = total + totalBruto;
                lblTotal.setText(total + "");
                int c1 = -1, c2 = -1, c3 = -1, c5 = -1;
                float f1 = 0, f2 = 0;
                c5 = inter.String_Int(txtTiendaID.getText());
                c1 = inter.String_Int(txtFacturaID.getText());
                c2 = inter.String_Int(txtArticuloID.getText());
                c3 = Integer.valueOf(txtCantidad.getText());
                f1 = inter.String_Float(lblPrecio.getText());
                f2 = inter.String_Float(lblISV.getText());
                detalle = new Detalle(c5, c1, c2, c3, f1, f2);
                Detalle.addElement(detalle);
                modelo.addRow(rou);
            } else {
                float ca = limite - temp;
                JOptionPane.showMessageDialog(rootPane, "No se puede agregar linea por que se excede\n limite establecido (25%) por la cantidad de " + ca);
            }
            totalBruto = 0;
            txtArticuloID.setText("");
            txtCantidad.setText("");
            lblNombreArticulo.setText("");
            lblPrecio.setText("");
            lblISV.setText("");
            chkISV.setSelected(false);
            txtArticuloID.grabFocus();
        }
    }//GEN-LAST:event_btnAgregarLineaActionPerformed

    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        dispose();
    }//GEN-LAST:event_jButton1ActionPerformed

    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        Limpiar_Campos();
    }//GEN-LAST:event_jButton2ActionPerformed

    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        if (!ins) {
            JOptionPane.showMessageDialog(rootPane, "El encabezado de la Compra no esta validado aun!!!", "Error", JOptionPane.ERROR_MESSAGE);
            valid.grabFocus();
        } else if (modelo.getRowCount() == 0) {
            JOptionPane.showMessageDialog(rootPane, "No se han ingresado Articulos en el detalle de la factura");
            txtArticuloID.grabFocus();
        } else {
            int res = JOptionPane.showConfirmDialog(rootPane, "¿Desea GRABAR ESTA FACTURA?", "PREGUNTA", JOptionPane.YES_NO_OPTION);
            if (res == JOptionPane.YES_OPTION) {
                String d = "";
                float _total = 0;
                if (cboTipo.getSelectedIndex() == 0) {
                    d = "X";
                    _total = inter.String_Float(lblTotal.getText());
                } else {
                    d = "Y";
                    _total = (float) (inter.String_Float(lblTotal.getText()) + (inter.String_Float(lblTotal.getText()) * 0.05));
                }
                FormaPago formapago = new FormaPago(factura.getTiendaID(), factura.getFacturaID(), d, _total);
                boolean y = control.insUniversal2(factura.getVectorData(), "Factura");
                boolean x = false;
                boolean w = false;
                for (Detalle o : Detalle) {
                    Existencia existencia = new Existencia();
                    Object oo[][] = control.seekUniversal(existencia.getVectorData(), "Existencia", o.getArticuloID() + " and TiendaID=" + factura.getTiendaID());
                    if (oo != null) {
                        existencia = new Existencia(oo);
                        int k = existencia.getCantidad() - o.getCantidad();
                        existencia.setCantidad(k);
                        w = control.updUniversal2(existencia.getVectorData(), "Existencia", existencia.getArticuloID() + " and TiendaID=" + existencia.getTiendaID());
                    }
                    x = control.insUniversal2(o.getVectorData(), "Detalle");
                    if (!x) {
                        break;
                    }
                    if (!w) {
                        break;
                    }
                }
                boolean z = control.insUniversal2(formapago.getVectorData(), "FormaPago");
                if ((y) && (x) && (z) && (w)) {
                    JOptionPane.showMessageDialog(rootPane, "FACTURA GRABADA CON EXITO");
                } else {
                    JOptionPane.showMessageDialog(rootPane, "OCURRIO UN ERROR AL GRABAR ESTA FACTURA, CONSULTE A SU DBA MANAGER");
                }
                totalBruto = 0;
                txtArticuloID.setText("");
                txtCantidad.setText("");
                lblNombreArticulo.setText("");
                lblPrecio.setText("");
                lblISV.setText("");
                chkISV.setSelected(false);
                Detalle.clear();
                for (int e = modelo.getRowCount() - 1; e > -1; e--) {
                    modelo.removeRow(e);
                }
                Limpiar_Campos();
            }
        }
    }//GEN-LAST:event_jButton3ActionPerformed
    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JTable Tabla;
    private javax.swing.JButton btnAgregarLinea;
    private javax.swing.JButton btnQuitarLinea;
    private javax.swing.JComboBox cboTipo;
    private javax.swing.JCheckBox chkISV;
    private javax.swing.JButton jButton1;
    private javax.swing.JButton jButton2;
    private javax.swing.JButton jButton3;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JLabel jLabel10;
    private javax.swing.JLabel jLabel11;
    private javax.swing.JLabel jLabel15;
    private javax.swing.JLabel jLabel16;
    private javax.swing.JLabel jLabel19;
    private javax.swing.JLabel jLabel22;
    private javax.swing.JLabel jLabel23;
    private javax.swing.JLabel jLabel24;
    private javax.swing.JLabel jLabel3;
    private javax.swing.JLabel jLabel4;
    private javax.swing.JLabel jLabel5;
    private javax.swing.JLabel jLabel6;
    private javax.swing.JLabel jLabel7;
    private javax.swing.JLabel jLabel8;
    private javax.swing.JLabel jLabel9;
    private javax.swing.JPanel jPanel1;
    private javax.swing.JPanel jPanel2;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JLabel lblDireccionTienda;
    private javax.swing.JLabel lblEstado;
    private javax.swing.JLabel lblISV;
    private javax.swing.JLabel lblNombreArticulo;
    private javax.swing.JLabel lblNombreSocio;
    private javax.swing.JLabel lblNombreTienda;
    private javax.swing.JLabel lblPrecio;
    private javax.swing.JLabel lblTotal;
    private javax.swing.JTextField txtArticuloID;
    private javax.swing.JTextField txtCantidad;
    private javax.swing.JTextField txtFacturaID;
    private javax.swing.JTextField txtFecha;
    private javax.swing.JTextField txtObservacion;
    private javax.swing.JTextField txtSocioID;
    private javax.swing.JTextField txtTiendaID;
    private javax.swing.JButton valid;
    // End of variables declaration//GEN-END:variables
}
